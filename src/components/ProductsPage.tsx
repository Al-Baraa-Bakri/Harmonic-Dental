...
const ProductsPage = ({
  ...
  const [selectedCategorySlug, setSelectedCategorySlug] = useState<string>(getInitialCategory);
  ...
  const filteredProducts =
    selectedCategorySlug === "all"
      ? productTypes
      : productTypes.filter((product: any) => 
          product.category?.name?.toLowerCase() === selectedCategorySlug.toLowerCase()
        );
      ...
      <section className="relative pt-32 pb-16 md:pt-40 md:pb-20 bg-gradient-to-b from-background via-background to-card/30 overflow-hidden">
        ...
        <div className="container mx-auto px-6 relative z-10">
          ...
          <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold mb-4">
            {heroTitle} <span className="text-gradient">{heroHighlightedText}</span>
          </h1>
          ...
          <div className="container mx-auto px-6">
            <div className="flex flex-nowrap overflow-x-auto gap-2 md:gap-3 scrollbar-thin scrollbar-thumb-border scrollbar-track-background">
              ...
              {categories.map((category: any) => (
                <Button
                  key={category.id}
                  variant={selectedCategorySlug.toLowerCase() === category.name.toLowerCase() ? "default" : "outline"}
                  onClick={() => setSelectedCategorySlug(category.name.toLowerCase())}
                  className="text-sm mb-3"
                >
                  {category.name}
                </Button>
                ...
                >
                  ...
                  <div className="relative h-64 md:h-80 2xl:h-96 overflow-hidden">
                    ...
                    {product.model3dUrl && (
                      ...
                      >
                        <p className="text-muted-foreground text-sm">3D Model View</p>
                    ...
                    {!isModelActive && product.category && (
                      <div className="absolute bottom-4 left-4 px-3 py-1 rounded-full bg-card/80 backdrop-blur-sm border border-primary/20 z-10">
                        <span className="text-xs text-foreground/90">{product.category.name}</span>
                      </div>
                    )}
                  </div>
